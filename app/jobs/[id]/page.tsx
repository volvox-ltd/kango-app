...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: job, error } = await supabase
    ...
    .select(`
      *,
      hospitals ( name, address, website_url, description, images ),
      applications ( status )
    `)
    .eq('id', id)
    .single();
  ...
  // ★ 現在の求人が満員かチェック
  // @ts-ignore
  const confirmedCount = job.applications?.filter((a: any) => ['confirmed', 'completed'].includes(a.status)).length || 0;
  // @ts-ignore
  const isCurrentFull = confirmedCount >= (job.capacity || 1);

  // 2. 同じグループの他日程を探す (+ それぞれの満員状況もチェック)
  let relatedJobs: any[] = [];
  if (job.group_id) {
    ...
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      <div className="w-full h-64 bg-gray-200 relative">
        ...
        {isCurrentFull && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-0">
            <span className="text-white font-bold border-2 border-white px-4 py-2 text-2xl transform -rotate-12">
              募集終了
            </span>
          </div>
        )}
      ...
      <div className="max-w-2xl mx-auto -mt-4 relative z-10 px-4">
              ...
              <div className="flex flex-wrap gap-2">
                {relatedJobs.map((relJob) => {
                  ...
                  if (relJob.isFull && !isCurrent) {
                    // 満員の場合はグレーアウト
                    return (
                      <div key={relJob.id} className="px-3 py-2 rounded-lg border bg-gray-100 text-gray-400 border-gray-200 text-sm cursor-not-allowed opacity-60">
                        <span className="block text-xs">{formatDate(relJob.start_time)}</span>
                        <span className="block font-bold text-xs mt-1">満員</span>
                      </div>
                    );
                  }
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-4 space-y-6">
          ...
          {job.precautions && (
            <section>
              ...
              <div className="bg-orange-50 p-3 rounded text-sm text-orange-800 whitespace-pre-wrap">{job.precautions}</div>
            </section>
          )}
          {job.document_url && (
            <section>
              ...
              <a href={job.document_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline text-sm">資料を確認する</a>
            </section>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-4">
          ...
          {job.access_info && <p className="text-xs text-gray-500 bg-gray-100 p-2 rounded mt-2">📍 {job.access_info}</p>}
          {/* @ts-ignore */}
          <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(job.hospitals?.address || '')}`} target="_blank" rel="noopener noreferrer" className="mt-4 block w-full border border-gray-300 text-center py-2 rounded text-sm text-gray-600 hover:bg-gray-50">Googleマップで開く</a>
        </div>
      </div>
